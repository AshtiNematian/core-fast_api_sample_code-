...
async def save_hour_1(args):
    ...
    while True:
        ...
        indicator = RSISMI(price_history)
        info = indicator.calculate()
        await mc.insert(crypto=symbol, time_frame=time_frame, algorithm=AlgorithmModel.RSI_SMI, info=info)
        await mc.clear_old_data(symbol=symbol, algorithm=AlgorithmModel.RSI_SMI, time_frame=time_frame)
        await change_checker(symbol, time_frame)
        await asyncio.sleep(6)


async def change_checker(symbol, time_frame):
    two_latest_actions = await mc.get_two_latest_actions(symbol=symbol, algorithm=AlgorithmModel.RSI_SMI,
                                                         time_frame=time_frame)
    check = check_signal_changed(two_latest_actions)
    if check != 0:
        message = f"New signal for %23RSISMI : %23{symbol} in %23{time_frame},\n" \
                  f"side : {check['action']}, close : {check['close']}, \n time : {check['date']}"
        # send_message_on_telegram(message)

    else:
        message = f"latest check in {datetime.datetime.now()} for %23RSISMI : %23{symbol} in %23{time_frame} "
