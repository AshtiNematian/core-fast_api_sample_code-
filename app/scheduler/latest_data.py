...
async def save_hour_1(args):
    ...
    while True:
        info = FetchKLineData(symbol=symbol, type=time_frame, limit='1').get()

        info['Time_'] = info.index
        info_dataframe = {'info': info[['Time_', 'Close', 'Volume', 'Amount']].to_dict(orient='records')}
        await mc.insert_price_history(crypto=symbol, time_frame=time_frame, limit='1', info=info_dataframe)
        await mc.clear_old_price_history(crypto=symbol, time_frame=time_frame, limit='1')
