...
@router.get("/fresh")
async def calculate_rsi_algorithm(symbol: str = "btcusdt", time_frame="1day", limit=100, ):
    ...
    {
        "message": f"return rsi strategy for {symbol} in {time_frame} limited by : {limit}",
        "symbol": symbol,
...
@router.get("/db")
async def get_rsi_algorithm(symbol: str = "btcusdt", time_frame="1hour", db: AsyncIOMotorClient = Depends(get_db), ):
    # username: str = Depends(get_current_username)):
    collection = f'{symbol}.{AlgorithmModel.RSI}.{time_frame}'
    ...
    if count_documents != 0:
        data = await get_actions_from_db(symbol, AlgorithmModel.RSI, time_frame, db)
        result = {
    ...
    return JSONResponse(status_code=status.HTTP_200_OK, content=
    {
        "message": f"return RSI strategy for {symbol} in {time_frame}",
        "symbol": symbol,
        "time_frame": time_frame,
        "body": result
    })
