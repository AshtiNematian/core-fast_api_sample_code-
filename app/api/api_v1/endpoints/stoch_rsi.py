...
@router.get("/fresh")
async def calculate_stoch_rsi_algorithm(symbol: str = "btcusdt", time_frame="1day", limit=100, ):
    # username: str = Depends(get_current_username)):
    data = FetchKLineData(symbol=symbol, type=time_frame, limit=limit).get()
    stoch_rsi = STOCHRSI(data)
    result = stoch_rsi.calculate()
    return JSONResponse(status_code=status.HTTP_200_OK, content=
    {
        "message": f"return stoch rsi strategy for {symbol} in {time_frame} limited by : {limit}",
        "symbol": symbol,
...
@router.get("/db")
async def get_stoch_rsi_algorithm(symbol: str = "btcusdt", time_frame="1day",
                                  db: AsyncIOMotorClient = Depends(get_db), ):
    # username: str = Depends(get_current_username)):
    collection = f'{symbol}.{AlgorithmModel.STOCH_RSI}.{time_frame}'
    ...
    if count_documents != 0:
        ...
        data = await get_actions_from_db(symbol, AlgorithmModel.STOCH_RSI, time_frame, db)
        result = {
